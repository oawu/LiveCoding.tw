...
$(function () {
  ...
  function getGPS (isFirst) {
    if (_isGet) return;
    _isGet = true;

    $.get ('http://127.0.0.1/livecoding.tw/gps.php', function (data) {
      console.error ('API');

      data.points = data.points.map (function (t) {
        return new google.maps.LatLng (t[0], t[1]);
      });
      _polyline.setPath (data.points);
      
      _user.setOptions ({
        // position: data.points[data.points.length - 1],
        labelContent: '<img src="img/user.png"><div class="comment">' + data.comment + '</div>'
      });
      ...
      if (isFirst === true) {
        ...
        console.error ('==========');
        markerGo (_user, data.points[data.points.length - 1]);
      }
  ...
  google.maps.event.addDomListener (window, 'load', function () {
    var position = new google.maps.LatLng (25.050935, 121.514819);
    ...
    $('#location').click (function () {
      _maps.setCenter (new google.maps.LatLng (_myPosition.lat, _myPosition.lng));
    });
    ...
    _user = new MarkerWithLabel ({
      ...
      labelClass: 'user',
        labelContent: '<img src="img/user.png">'
    });
    ...
    // setTimeout (function () {
    //   _user.setOptions ({
    //     labelContent: '<img src="img/user.png"><div class="comment">123</div>'
    //   });
    // }, 5 * 1000);
